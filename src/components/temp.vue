<template>
	<div class="card">
		...
		<div class="flex-grid">
			...
			<div v-if="user.favorites[this.bar['.key']]" style="border:1px orange solid;padding: 2px; margin:8px">
						<!-- if this is a user and is in bar favorites -->
...
// import asyncDataStatus from '@/mixins/asyncDataStatus'
	...
	...mapActions('bars', ['fetchFavorites', 'createBarFavorite', 'deleteBarFavorites', 'fetchFavorite', 'fetchAllBars']),
      storeMessage () {
        firebase.database().ref('messages').push({text: this.messageText, nickname: this.nickname})
        this.messageText = ''
      },
      ...
      updateMessage () {
        firebase.database().ref('messages').child(this.editingMessage.id).update({text: this.messageText})
        this.cancelEditing()
      }
    },
    created () {
	...
	computed: {
		...
		// 		.filter(favorite => favorite.barId === this.bar['.key'])
		// },
		// users () {
...
<script>
	...
	data () {
		return {
			...
			userFave: '',
			// fave: ''
			fave: this.bar.favorite ? this.bar.favorite : ''
	...
	created () {
		...
		this.$emit('ready')
			// this.fetchFavorite()
			// 	// .then(bars => Promise.all(bars.map(bar => this.fetchFavorites({ids: Object.keys(bar.favorites)}))))
			// 	.then(() => {
